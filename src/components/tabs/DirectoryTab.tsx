...
export const DirectoryTab: React.FC<DirectoryTabProps> = ({
  ...
  // Cara bergabung klaster: kode undangan (Join with Code) atau buat/pilih klaster via nama
  const [joinMode, setJoinMode] = useState<'CODE' | 'NAME'>('CODE');
  const [inviteCode, setInviteCode] = useState('WJW-MENTNG');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    onAddUser({
      ...
      bio,
      language,
      inviteCode: joinMode === 'CODE' ? inviteCode : undefined,
      cluster: joinMode === 'NAME' ? clusterName : undefined,
    });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl text-white space-y-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              {/* CARA BERGABUNG KLASTER: KODE UNDANGAN atau BUAT/PILIH KLASTER */}
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1">
                  Cara Bergabung Klaster
                </label>
                ...
                {joinMode === 'CODE' ? (
                  <div>
                    <input
                      ...
                      placeholder="Contoh: WJW-MENTNG"
                      className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white font-black tracking-widest focus:outline-none focus:border-indigo-500"
                    />
                    <p className="text-[10px] text-slate-500 mt-1">
                      Minta kode undangan dari Admin Klaster Anda (Join with Code). Demo: WJW-MENTNG,
                      WJW-KBYRAN, WJW-KLPGDG.
                    </p>
                  </div>
                ) : (
                  <div>
                    <input
                      ...
                      placeholder="Contoh: Klaster Griya Asri Soreang"
                    ...
                    />
                    <p className="text-[10px] text-slate-500 mt-1">
                      Jika nama klaster belum terdaftar, klaster BARU akan dibuat — Anda menjadi Admin
                      awal dan mendapat kode undangan untuk dibagikan.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-1">
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  WargaJagaWarga adalah aplikasi berbayar PER-KLASTER yang diawasi oleh Superadmin
                  (tarafk1972@gmail.com). Setiap klaster baru mendapat masa percobaan GRATIS 14 hari,
                  setelah itu Admin memilih paket Bulanan (Rp 150.000/klaster) atau Tahunan (Rp
                  1.500.000/klaster). Peringatan billing hanya ditampilkan kepada Admin klaster —
                  Emergency Alert warga tidak pernah diblokir.
                </p>
              </div>
