...
import { getIncidents, createIncident, checkBillingForSOS } from '@/lib/db';
...
export async function POST(req: Request) {
  try {
    ...
    // ===== CEK BILLING KLASTER SEBELUM KIRIM SOS (sesuai alur kerja) =====
    // Kedaluwarsa / ditangguhkan -> Emergency Alert DIBLOKIR
    // Trial hampir habis -> tetap kirim + PERINGATAN
    const billingCheck = checkBillingForSOS(cluster || 'Klaster Menteng Asri');
    ...
    return NextResponse.json(
      { incident: newIncident, billingWarning: billingCheck.warning || null },
      { status: 201 }
    );
  } catch {
