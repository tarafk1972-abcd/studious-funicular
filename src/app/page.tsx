...
function HomeInner() {
  ...
  // Handle creating new SOS Incident (dengan CEK BILLING KLASTER sesuai alur kerja)
  const handleCreateSOS = async (data: {
    ...
    try {
      ...
      if (res.status === 402) {
        // BILLING DIBLOKIR: Emergency Alert tidak dikirim
        const err = await res.json();
        setBillingNotice(err.blockedReason || 'Emergency Alert diblokir karena masalah billing klaster.');
      ...
      if (res.ok) {
        ...
        if (result.billingWarning) {
          // Trial hampir habis: SOS tetap terkirim + peringatan
          setBillingNotice(result.billingWarning);
        }
        playSirenChime();
        await fetchAllState();
        setActiveTab('app');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      {/* 2b. NOTIFIKASI BILLING (peringatan trial / SOS diblokir) */}
      {billingNotice && (
