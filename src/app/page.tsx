...
function HomeInner() {
  ...
  // ===== LANGGANAN APLIKASI BERBAYAR (Percobaan Gratis 14 Hari) =====
  // Simulasi pembayaran paket Bulanan / Tahunan
  const handlePaySubscription = async (userId: string, plan: SubscriptionPlan) => {
    try {
      const res = await fetch('/api/subscriptions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'pay', userId, plan }),
      });
      if (res.ok) {
        confetti({
          particleCount: 90,
  ...
  // Pengawasan langganan oleh Superadmin (Customer Service)
  const handleManageSubscription = async (
    userId: string,
    manageAction: 'EXTEND_TRIAL' | 'SUSPEND' | 'ACTIVATE'
  ) => {
    if (!currentUser) return;
    try {
      const res = await fetch('/api/subscriptions', {
