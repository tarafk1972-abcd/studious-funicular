...
// Cek kelayakan billing klaster sebelum warga mengirim SOS (sesuai alur PDF):
// - AKTIF / TRIAL berjalan  -> boleh kirim
// - TRIAL sisa <= 3 hari    -> boleh kirim + PERINGATAN
// - KEDALUWARSA / DITANGGUHKAN -> DIBLOKIR
export function checkBillingForSOS(clusterName: string): {
  allowed: boolean;
  warning?: string;
  ...
  if (!cluster) {
    // Klaster tak dikenal (mis. Superadmin lintas klaster) — izinkan
    return { allowed: true };
  }
  ...
  if (billing.status === 'DITANGGUHKAN') {
    return {
      allowed: false,
      blockedReason: `Billing klaster ${cluster.name} sedang DITANGGUHKAN oleh Superadmin. Emergency Alert diblokir. Hubungi Customer Service (tarafk1972@gmail.com).`,
      billing,
    };
  }
  if (billing.status === 'KEDALUWARSA') {
    return {
      allowed: false,
      blockedReason: `Langganan klaster ${cluster.name} telah KEDALUWARSA. Emergency Alert diblokir sampai Admin melunasi tagihan (Rp ${billing.amountDue.toLocaleString('id-ID')}).`,
      billing,
    };
  }
  if (billing.status === 'TRIAL') {
    ...
    if (daysLeft <= 3) {
      return {
        allowed: true,
        warning: `Masa percobaan klaster ${cluster.name} tersisa ${daysLeft} hari. Segera minta Admin memilih paket agar Emergency Alert tidak terblokir.`,
        billing,
      };
    }
  }
